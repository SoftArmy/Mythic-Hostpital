...
function ProcessRunStuff(ped)
    if IsInjuryCausingLimp() and not (onPainKiller > 0)  then
        ...
        if wasOnPainKillers then
            ...
            exports['mythic_notify']:SendAlert('inform', 'You realize that taking medication does not solve all your problems', 5000, { ['background-color'] = '#760036' })
        end
    else
            ...
            elseif (v.part == 'HEAD' and v.severity > 2) then
                if headCount >= Config.HeadInjuryTimer then
                    ...
                    if chance <= Config.HeadInjuryChance then
                        exports['mythic_notify']:SendAlert('inform', 'You have a blackout', 5000, { ['background-color'] = '#760036' })
...
function DoBleedAlert()
    ...
    if not IsEntityDead(player) and isBleeding > 0 then
        exports['mythic_notify']:PersistentHudText('start', bleedNotifId, 'inform', 'U heeft ' .. Config.BleedingStates[isBleeding], { ['background-color'] = '#760036' })
    else
...
AddEventHandler('mythic_hospital:client:UsePainKiller', function(tier)
    ...
    exports['mythic_notify']:SendAlert('inform', 'You feel the pain go away temporarily', 5000)
    ProcessRunStuff(PlayerPedId())
end)
...
AddEventHandler('mythic_hospital:client:UseAdrenaline', function(tier)
    ...
    exports['mythic_notify']:SendAlert('inform', 'You are not able to ignore your body when it fails', 5000)
    ProcessRunStuff(PlayerPedId())
end)
        ...
        if bleedTickTimer >= Config.BleedTickRate then
            ...
            if math.floor(bleedTickTimer % (Config.BleedTickRate / 10)) == 0 then
                ...
                if (moving > 1 and not IsPedInAnyVehicle(player)) and isBleeding > 2 then
                    exports['mythic_notify']:PersistentHudText('start', bleedMoveNotifId, 'inform', 'You notice that blood drain faster from your wounds when you move', { ['background-color'] = '#4d0e96' })
